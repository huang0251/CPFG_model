...
def single_distance(chord1, chord2):
    if len(chord1) == 0 or len(chord2) == 0:
        raise 'Error: empty chord.'
    scale = ['C','Db','D','Eb','E','F','Gb','G','Ab','A','Bb','B']
    chords_list = []
    ...
    chord1 = chords_list[0]
    chord2 = chords_list[1]
    ...
    def get_set(chord):
        ...
        if len(tones) > 1:
            ...
            chords = toneObjList[0]
        #print(len(chords))
        return chords

    mindis = 100000.
    for i in get_set(chord1):
        for j in get_set(chord2):
            d = i.position.distance(j.position)
            if d < mindis:
                mindis = d
    return mindis