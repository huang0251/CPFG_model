...
class CVAE_GRU(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.encode_linear_to_attention = torch.nn.Linear(CURVE_SIZE + MELODY_SIZE, 512)
        self.encode_attention = torch.nn.MultiheadAttention(512, ATT_HEADS, dropout=ATT_DROPOUT, batch_first=True)
        self.encode_attention2 = torch.nn.MultiheadAttention(512, ATT_HEADS, dropout=ATT_DROPOUT, batch_first=True)
        ...
        self.decode_attention2 = torch.nn.MultiheadAttention(512, ATT_HEADS, dropout=ATT_DROPOUT, batch_first=True)
        ...
        self.linear_mu = torch.nn.Linear(512, Z_SIZE)
        self.linear_std = torch.nn.Linear(512, Z_SIZE)

        self.linear_tension_out_0 = torch.nn.Linear(OUTPUT_EMB_SIZE, OUTPUT_EMB_SIZE//2)
        self.linear_distance_out_0 = torch.nn.Linear(OUTPUT_EMB_SIZE, OUTPUT_EMB_SIZE//2)
        self.linear_strain_out_0 = torch.nn.Linear(OUTPUT_EMB_SIZE, OUTPUT_EMB_SIZE//2)
        self.linear_int_out_0 = torch.nn.Linear(OUTPUT_EMB_SIZE, OUTPUT_EMB_SIZE//2)

        self.linear_tension_out = torch.nn.Linear(OUTPUT_EMB_SIZE//2, 1)
        self.linear_distance_out = torch.nn.Linear(OUTPUT_EMB_SIZE//2, 1)
        self.linear_strain_out = torch.nn.Linear(OUTPUT_EMB_SIZE//2, 1)
        self.linear_int_out = torch.nn.Linear(OUTPUT_EMB_SIZE//2, KEY_SIZE)
    ...
    def encoder(self, x, lens, generate=False):
        ...
        if not generate:
            ...
            x_attn = self.norm2(x_attn + x)
            x_attn, _ = self.encode_attention2(x_attn, x_attn, x_attn, key_padding_mask=x_padding_mask)
        else:
            x = self.linear_dropout(x)
            x_attn, _ = self.encode_attention(x, x, x)
            x_attn = self.norm2(x_attn + x)
            x_attn, _ = self.encode_attention2(x_attn, x_attn, x_attn)
        x_attn = x_attn.mean(dim=1)
        
        gru_emb = self.linear_dropout(x_attn)
    ...
    def decoder(self, z, c1, lens, generate):
        ...
        if not generate:
            ...
            emb_attn = self.norm1(emb_attn + gru_emb)
            emb_attn, _ = self.decode_attention2(emb_attn, emb_attn, emb_attn, key_padding_mask=emb_padding_mask)
        else:
            gru_emb = self.linear_dropout(gru_emb)
            emb_attn, _ = self.decode_attention(gru_emb, gru_emb, gru_emb)
            emb_attn = self.norm1(emb_attn + gru_emb)
            emb_attn, _ = self.decode_attention2(emb_attn, emb_attn, emb_attn)
