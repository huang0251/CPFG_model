...
class CPFG(torch.nn.Module):
    ...
    def __init__(self):
        ...
        self.linear_int_out_0 = torch.nn.Linear(Z_SIZE + 2*GRU_NUM_LAYERS*MELODY_EMB_SIZE, OUTPUT_EMB_SIZE)
    ...
    def decoder(self, z, c1, lens, generate):
        ...
        if not generate:
            output_i = F.leaky_relu(self.linear_int_out_0(torch.cat([z, c1_gru_emb.view(BATCH_SIZE, -1)], dim=-1)))
        else:
            output_i = F.leaky_relu(self.linear_int_out_0(torch.cat([z, c1_gru_emb.view(1, -1)], dim=-1)))
        ...
        output = torch.cat([output_tension, output_distance, output_strain, output_int.unsqueeze(1).repeat(1, maxlen, 1)], dim=-1)
...
#Generate functions
    ...
    def generate_batch(self, batch):
        ...
        cond1_hot_part = batch['melody'][..., 0].long()
        cond1_onehot_part = F.one_hot(cond1_hot_part, num_classes = PITCH_SIZE).float()

        cond1_int_part = batch['melody'][..., 1].float().unsqueeze(-1)
        SOS = torch.ones(cond1_int_part.size(0), cond1_int_part.size(1), 1).to(cond1_int_part.device).float()

        c1 = torch.cat([SOS, cond1_onehot_part, SOS, cond1_int_part], dim=-1)

        #print(c1.size(), c2.size())
        z = torch.randn(c1.size(0), Z_SIZE).to(c1.device)
